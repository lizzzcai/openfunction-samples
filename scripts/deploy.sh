...
if [ "$with_cert_manager" = "true" ]; then
  if [ "$region_cn" = "false" ]; then
    ...
    kubectl apply --filename https://openfunction.sh1a.qingstor.com/cert-manager/${cert_manager_version}/cert-manager.yaml
  fi
fi
...
if [ "$with_shipwright" = "true" ]; then
  if [ "$region_cn" = "false" ]; then
    ...
    kubectl apply --filename https://github.com/tektoncd/pipeline/releases/download/${tekton_pipeline_version}/release.yaml
    ...
    # Install Tekton pipeline
    kubectl apply --filename https://openfunction.sh1a.qingstor.com/tekton/pipeline/${tekton_pipeline_version}/release.yaml
    # Install the Shipwright deployment
    kubectl apply --filename https://openfunction.sh1a.qingstor.com/shipwright/${shipwright_version}/release.yaml
  fi
fi
...
if [ "$with_knative" = "true" ]; then
  if [ "$region_cn" = "false" ]; then
    ...
    # Install the required custom resources
    kubectl apply -f https://openfunction.sh1a.qingstor.com/knative/serving/knative-${knative_version}/serving-crds.yaml
    # Install the core components of Serving
    kubectl apply -f https://openfunction.sh1a.qingstor.com/knative/serving/knative-${knative_version}/serving-core.yaml
    # Install a networking layer
    # Install the Knative Kourier controller
    kubectl apply -f https://openfunction.sh1a.qingstor.com/knative/net-kourier/knative-${kourier_version}/kourier.yaml
    # To configure Knative Serving to use Kourier by default
    # kubectl patch configmap/config-network \
    #   --namespace knative-serving \
    #   --type merge \
    #   --patch '{"data":{"ingress.class":"kourier.ingress.networking.knative.dev"}}'
    # Configure DNS
    # kubectl apply -f https://openfunction.sh1a.qingstor.com/knative/serving/${knative_version}/serving-default-domain.yaml
  fi
fi
...
if [ "$with_openFuncAsync" = "true" ]; then
  if [ "$region_cn" = "false" ]; then
    ...
    # Init dapr
    dapr init -k --runtime-version ${dapr_version}
    ...
    dapr init -k --runtime-version ${dapr_version}
    # Installs the latest release version
    kubectl apply -f https://openfunction.sh1a.qingstor.com/keda/v${keda_version}/keda-${keda_version}.yaml
  fi
fi
